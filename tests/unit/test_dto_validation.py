...
class TestUserRegisterDTO:
    def test_valid_data(self):
        dto = UserRegisterDTO(email="test@test.com", full_name="Іван Тест", password="pass123")
        assert dto.email == "test@test.com"

    def test_short_password_raises(self):
        with pytest.raises(ValidationError):
            UserRegisterDTO(email="test@test.com", full_name="Тест", password="123")

    def test_invalid_email_raises(self):
        with pytest.raises(ValidationError):
            UserRegisterDTO(email="not-an-email", full_name="Тест", password="pass123")

    def test_empty_name_raises(self):
        with pytest.raises(ValidationError):
            UserRegisterDTO(email="test@test.com", full_name="   ", password="pass123")

    def test_password_exactly_6_chars_ok(self):
        dto = UserRegisterDTO(email="a@b.com", full_name="Тест", password="123456")
        assert dto.password == "123456"

    def test_password_5_chars_raises(self):
        with pytest.raises(ValidationError):
            UserRegisterDTO(email="a@b.com", full_name="Тест", password="12345")

    def test_valid_phone(self):
        dto = UserRegisterDTO(email="a@b.com", full_name="Тест", password="pass123", phone="+380501234567")
        assert dto.phone == "+380501234567"

    def test_invalid_phone_raises(self):
        with pytest.raises(ValidationError):
            UserRegisterDTO(email="a@b.com", full_name="Тест", password="pass123", phone="abc")

    def test_phone_none_allowed(self):
        dto = UserRegisterDTO(email="a@b.com", full_name="Тест", password="pass123", phone=None)
        assert dto.phone is None

    def test_missing_email_raises(self):
        with pytest.raises(ValidationError):
            UserRegisterDTO(full_name="Тест", password="pass123")
    ...
    def test_long_password_ok(self):
        dto = UserRegisterDTO(email="a@b.com", full_name="Тест", password="a" * 50)
        assert len(dto.password) == 50

    def test_special_chars_in_name_ok(self):
        dto = UserRegisterDTO(email="a@b.com", full_name="Іван-Петро О'Брайен", password="pass123")
        assert dto.full_name == "Іван-Петро О'Брайен"

    def test_email_is_valid_string(self):
        dto = UserRegisterDTO(email="User@Example.Com", full_name="Тест", password="pass123")
        assert "@" in dto.email


class TestUserLoginDTO:
    def test_valid_login(self):
        dto = UserLoginDTO(email="user@test.com", password="pass123")
        assert dto.email == "user@test.com"

    def test_missing_email_raises(self):
        with pytest.raises(ValidationError):
            UserLoginDTO(password="pass123")
    ...
    def test_invalid_email_raises(self):
        with pytest.raises(ValidationError):
            UserLoginDTO(email="notvalid", password="pass123")
...
class TestLocationCreateDTO:
    def test_valid_data(self):
        ...
        assert dto.price_per_hour == 200.0
    ...
    def test_large_price_ok(self):
        ...
        assert dto.price_per_hour == 9999.99
...
class TestLocationUpdateDTO:
    ...
    def test_update_price_only(self):
        ...
        assert dto.price_per_hour == 500.0
